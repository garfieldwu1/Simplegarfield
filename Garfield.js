...
console.log(chalk.bold.hex("#ff03ea").bold("[ Mark Hitsuraan ] » ") + chalk.bold.hex("#fcba03").bold("Initializing variables..."));
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Garfield BOT`, '[ Mark Hitsuraan ]');
        logger('Hey, thank you for using this Bot file', '[ Mark Hitsuraan ]');
        logger("Fixed and modified by Mark Hitsuraan", '[ Mark Hitsuraan ]');
        logger("Facebook : www.facebook.com/markqtypie", 'Fixed by Mark Hitsuraan');
      var cron = require("node-cron");
...
cron.schedule('0 0 15 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage(`𝖡𝗂𝖻𝗅𝖾 𝗏𝖾𝗋𝗌𝖾𝗌 𝖿𝗈𝗋 𝗍𝗈 𝖽𝖺𝗒 🔔\n\n${textt}"\n\n- ${namet}`, now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
